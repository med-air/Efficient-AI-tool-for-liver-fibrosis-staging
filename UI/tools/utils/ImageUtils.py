...
def resizeBySpacing_GT(image: ndarray, shape: Tuple[int, int], spacing: Tuple[float, float]) -> ndarray:
    
    resized = cv2.resize(image, (200, 200), interpolation = cv2.INTER_NEAREST)
    return padImage(resized, shape)
